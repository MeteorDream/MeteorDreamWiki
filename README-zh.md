...
|---|---|---|---|
| **原始资料（Raw sources）** | 你 | 文章、论文、Agent 对话记录、网页剪藏 —— 不可变，是真理之源 | `source/`（已存档输入）、`_raw/`（中转收件箱）、`~/.claude`、`~/.codex`、`~/.hermes`，加上 `OBSIDIAN_SOURCES_DIR` 里指向的任何目录 |
...
> **为什么 `source/` 与 `_raw/` 要分开。** `_raw/` 是新资料进入 vault 的*收件箱*；`source/` 是它们摄入完成之后定居下来的*永久存档*。这种切分让 `wiki-ingest` 的规则保持简单：「`_raw/` 里的一切都是未处理的」；同时把每一字节的来源凭证保留在 `source/<主题>/`。任何时候你都可以 grep、重读、或者重新摄入这些已存档的资料 —— vault 永远不会悄悄丢数据。
...
| `source/` | **所有曾摄入过的原始资料的永久存档**，按主题分目录存放（如 `source/llm/knowledge/`、`source/web-clippings/<host>/`、`source/transcripts/<agent>/`）。Markdown 资料会被附加存档 frontmatter（存档时间、来源 URL、内容哈希、产出的 wiki 页面）；二进制资料则配套写一个 `.meta.yaml`。**视为只读历史**。 |
| `_raw/` | 把新资料丢这里；`wiki-ingest` 会处理它，并把**原文件移动到 `source/`** 而**不是删除**。 |
| `_staging/` | 当 `WIKI_STAGED_WRITES=true` 时，AI 写入的页面会先进入这里供你审核 |
| `_archives/` | 重建/恢复操作的快照存档 *（注意：与 `source/` 不同 —— 后者存的是原始输入，本目录存的是 wiki 自身的快照）* |
...
- **`_raw/` 是中转的收件箱；`source/` 是永久存档。** `wiki-ingest` 从 `_raw/` 读入资料、蒸馏成 wiki 页面之后，会把**原文件移动到 `source/<主题>/`**（Markdown 文件附加存档 frontmatter，二进制文件配 `.meta.yaml`），**而不是删除**。资料的来源凭证永远留在 vault 里。
